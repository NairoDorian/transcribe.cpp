...
std::vector<AudioChunkSpan> plan_quiet_energy_audio_chunks(const float * pcm,
    ...
    while (chunk_start < n_samples) {
        ...
        const int64_t search_start = std::max<int64_t>(chunk_start + frame_size,
                                                       chunk_start + target_chunk_samples - search_window_samples);
        const int64_t search_end = std::min<int64_t>(n_samples - frame_size,
                                                     chunk_start + target_chunk_samples);
...
void rebase_speaker_segments(std::vector<transcribe_session::SpeakerSegmentEntry> & spk_segments,
                             int64_t                                               offset_ms) {
...
transcribe_session::ResultSet merge_chunk_results(const std::vector<transcribe_session::ResultSet> & chunk_results,
    ...
    for (size_t i = 0; i < chunk_results.size(); ++i) {
        ...
        if (!cr.full_text.empty()) {
            if (!merged.full_text.empty() &&
                merged.full_text.back() != ' ' &&
                cr.full_text.front() != ' ') {
                merged.full_text += " ";
            }
        ...
        if (!cr.raw_text.empty()) {
            if (!merged.raw_text.empty() &&
                merged.raw_text.back() != ' ' &&
                cr.raw_text.front() != ' ') {
                merged.raw_text += " ";
            }
