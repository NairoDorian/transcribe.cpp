...
VerifyBuild build_verify_graph(ggml_context *                   ctx,
    ...
    if (ctx == nullptr || T_verify <= 0 || max_n_kv <= 0) {
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "qwen3_asr verify: invalid arg (T_verify=%d max_n_kv=%d)", T_verify,
                max_n_kv);
        return vb;
    }
    if (kv_cache.self_k == nullptr) {
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "qwen3_asr verify: kv_cache not initialized");
        return vb;
    }
    if (max_n_kv > kv_cache.n_ctx) {
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "qwen3_asr verify: max_n_kv=%d exceeds kv_cache.n_ctx=%d", max_n_kv,
                kv_cache.n_ctx);
        return vb;
    }
    ...
    ggml_cgraph * gf = ggml_new_graph_custom(ctx, /*size=*/8192, /*grads=*/false);
    if (gf == nullptr) {
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "qwen3_asr verify: ggml_new_graph_custom failed");
        return vb;
    }
    vb.graph = gf;

    ggml_tensor * x = ggml_get_rows(ctx, weights.dec_embed.token_w,
                                    vb.input_ids_in);  // [hidden, T_verify]

    for (int il = 0; il < n_layer; ++il) {
        x = causal_lm::block_step_n(ctx, gf, x, to_block_view(weights.dec_blocks[il]), block_params, kv_cache, il,
                                    T_verify, max_n_kv, vb.mask_in, vb.positions_in, vb.kv_idx_in, use_flash);
    }

    x                    = ggml_mul(ctx, ggml_rms_norm(ctx, x, rms_eps), weights.dec_final.norm_w);
    ggml_tensor * logits = ggml_mul_mat(ctx, weights.dec_embed.token_w, x);  // [vocab, T_verify]
    ggml_set_name(logits, "verify.logits");

    // Per-position argmax on-device → T_verify-int readback.
    ggml_tensor * amax = ggml_argmax(ctx, logits);
    ggml_set_name(amax, "verify.argmax");
    vb.out = amax;
