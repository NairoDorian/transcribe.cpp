...
const cJSON * field(const cJSON * obj, const char * name) {
    ...
    if (!v) {
        throw std::runtime_error(std::string("R2T2 missing field: ") + name);
    }
    return v;
}
...
uint32_t number(const cJSON * obj, const char * name) {
    ...
    if (!cJSON_IsNumber(v) || !std::isfinite(v->valuedouble) || v->valuedouble < 0 || v->valuedouble > UINT32_MAX ||
...
const char * str(const cJSON * v) {
    if (!cJSON_IsString(v) || !v->valuestring) {
        throw std::runtime_error("R2T2 expected JSON string");
    }
...
Json sidecar(gguf_context * g, const char * name) {
    ...
    if (gguf_get_arr_n(g, ok) != n + 1) {
        throw std::runtime_error("R2T2 invalid sidecar offsets");
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        if (std::strcmp(gguf_get_arr_str(g, nk, i), name) != 0) {
            continue;
        }
        ...
        Json j(cJSON_ParseWithLength(data + offsets[i], static_cast<size_t>(offsets[i + 1] - offsets[i])),
               cJSON_Delete);
        if (!j) {
            throw std::runtime_error(std::string("R2T2 invalid JSON: ") + name);
        }
        return j;
    }
...
transcribe_status prepare_r2t2_metadata(gguf_context * g) {
    try {
        auto   config  = sidecar(g, "config.json");
        auto   tok     = sidecar(g, "tokenizer.json");
        auto * thinker = cJSON_GetObjectItemCaseSensitive(config.get(), "thinker_config");
        if (!thinker) {
            thinker = config.get();
        }
        auto * enc = field(thinker, "audio_config");
        auto * dec = field(thinker, "text_config");
        auto   u   = [&](const std::string & k, uint32_t v) {
            gguf_set_val_u32(g, k.c_str(), v);
        };
        auto s = [&](const std::string & k, const char * v) {
            gguf_set_val_str(g, k.c_str(), v);
        };
        auto f = [&](const std::string & k, float v) {
            gguf_set_val_f32(g, k.c_str(), v);
        };
        const std::pair<const char *, const char *> emap[] = {
            ...
            { "downsample_hidden", "downsample_hidden_size"  },
        ...
        for (auto kv : emap) {
            u(std::string("stt.qwen3_asr.encoder.") + kv.first, number(enc, kv.second));
        }
        u("stt.qwen3_asr.encoder.max_source_positions",
          cJSON_GetObjectItemCaseSensitive(enc, "max_source_positions") ? number(enc, "max_source_positions") : 1500);
        ...
        for (auto kv : dmap) {
            u(std::string("stt.qwen3_asr.decoder.") + kv.first, number(dec, kv.second));
        }
        ...
        if (!rope) {
            rope = field(field(dec, "rope_parameters"), "rope_theta");
        }
        f("stt.qwen3_asr.decoder.rope_theta", static_cast<float>(rope->valuedouble));
        u("stt.qwen3_asr.decoder.rope_mrope_section_t", 24);
        u("stt.qwen3_asr.decoder.rope_mrope_section_h", 20);
        u("stt.qwen3_asr.decoder.rope_mrope_section_w", 20);
        ...
        if (tied && cJSON_IsFalse(tied)) {
            throw std::runtime_error("R2T2 untied output projection is unsupported");
        }
        gguf_set_val_bool(g, "stt.qwen3_asr.decoder.tie_word_embeddings", true);
        for (auto k : { "audio_token_id", "audio_start_token_id", "audio_end_token_id" }) {
            u(std::string("stt.qwen3_asr.") + k, number(thinker, k));
        }
        s("stt.variant", k_r2t2_variant);
        s("stt.frontend.type", "mel");
        u("stt.frontend.sample_rate", 16000);
        u("stt.frontend.num_mels", 128);
        u("stt.frontend.n_fft", 400);
        u("stt.frontend.win_length", 400);
        u("stt.frontend.hop_length", 160);
        s("stt.frontend.window", "hann_periodic");
        s("stt.frontend.normalize", "per_utterance");
        s("stt.frontend.pad_mode", "reflect");
        s("stt.frontend.mel_norm", "slaney");
        gguf_set_val_bool(g, "stt.frontend.center", true);
        f("stt.frontend.pre_emphasis", 0);
        f("stt.frontend.dither", 0);
        f("stt.frontend.f_min", 0);
        f("stt.frontend.f_max", 8000);
        u("stt.frontend.chunk_length", 30);
        u("stt.frontend.n_samples", 480000);
        u("stt.frontend.nb_max_frames", 3000);
        uint32_t vocab = number(dec, "vocab_size");
        if (vocab < 1000 || vocab > 1000000) {
            throw std::runtime_error("R2T2 invalid vocabulary size");
        }
        std::vector<std::string> tokens(vocab);
        ...
        for (auto * item = added->child; item; item = item->next) {
            auto id = number(item, "id");
            if (id >= vocab) {
                throw std::runtime_error("R2T2 added token out of range");
            }
            tokens[id] = str(field(item, "content"));
            types[id]  = 3;
        }
        std::vector<std::string> merges;
        for (auto * item = field(model, "merges")->child; item; item = item->next) {
            if (cJSON_IsString(item)) {
                merges.emplace_back(str(item));
            } else if (cJSON_IsArray(item) && cJSON_GetArraySize(item) == 2) {
                merges.emplace_back(std::string(str(cJSON_GetArrayItem(item, 0))) + " " +
                                    str(cJSON_GetArrayItem(item, 1)));
            } else {
                throw std::runtime_error("R2T2 invalid BPE merge");
            }
        }
        s("tokenizer.ggml.model", "gpt2");
        s("tokenizer.ggml.pre", "qwen2");
        strings(g, "tokenizer.ggml.tokens", tokens);
        strings(g, "tokenizer.ggml.merges", merges);
        gguf_set_arr_data(g, "tokenizer.ggml.token_type", GGUF_TYPE_INT32, types.data(), types.size());
        for (uint32_t i = 0; i < vocab; ++i) {
            if (tokens[i] == "<|im_end|>") {
                u("tokenizer.ggml.eos_token_id", i);
            }
            if (tokens[i] == "<|endoftext|>") {
                u("tokenizer.ggml.padding_token_id", i);
            }
        }
        strings(g, "general.languages",
        ...
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "R2T2 package: %s", e.what());
...
std::vector<std::pair<ggml_tensor *, std::string>> rename_r2t2_tensors(ggml_context * ctx) {
    ...
    for (auto * t = ggml_get_first_tensor(ctx); t; t = ggml_get_next_tensor(ctx, t)) {
        ...
        if (mapped != t->name) {
            old.emplace_back(t, t->name);
            ggml_set_name(t, mapped.c_str());
        }
    }
    return old;
}
...
transcribe_status plan_r2t2_dtypes(ggml_context * src_ctx, ggml_context ** out_ctx) {
    if (src_ctx == nullptr || out_ctx == nullptr) {
    ...
    *out_ctx = nullptr;
    ...
    log_msg(TRANSCRIBE_LOG_LEVEL_INFO, "R2T2 package: planned %zu of %zu tensors as F32 (BF16 in the file)", promoted,
            n);
    *out_ctx = ctx;
    return TRANSCRIBE_OK;
}
