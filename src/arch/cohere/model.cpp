...
transcribe_status load(Loader & loader, const transcribe_model_load_params * params, transcribe_model ** out_model) {
    ...
    if (weights_buffer == nullptr) {
        gguf_free(gguf_data);
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "cohere: alloc_ctx_tensors_with_reclaim failed");
        return TRANSCRIBE_ERR_GGUF;
    }
            ...
            if (new_compute_ctx(4 * 1024 * 1024)) {
                ...
                if (db_reserve.graph != nullptr) {
                    if (!ggml_backend_sched_reserve(cc->sched, db_reserve.graph)) {
                        log_msg(TRANSCRIBE_LOG_LEVEL_WARN, "cohere run: ggml_backend_sched_reserve failed");
                    }
                }
            }
