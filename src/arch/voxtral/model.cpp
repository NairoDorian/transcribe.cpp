...
transcribe_status load(Loader & loader, const transcribe_model_load_params * params, transcribe_model ** out_model) {
    ...
    if (weights_buffer == nullptr) {
        gguf_free(gguf_data);
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR, "voxtral: alloc_ctx_tensors_with_reclaim failed");
        return TRANSCRIBE_ERR_GGUF;
    }
...
transcribe_status run(transcribe_session *          session,
    ...
    // Free GPU buffers (KV cache + scheduler galloc) after each transcription to
